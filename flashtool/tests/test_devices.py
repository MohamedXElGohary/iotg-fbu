...
def test_lsusb_get_devices(serial_no):
    ...
    assert devices[0]["iSerial"] == serial_no

def test_fastboot_get_devices(serial_no):
    with mock.patch("flashtool.subprocess") as mocked:
        stdout = bytes("{}\tfastboot\n".format(serial_no), "utf-8")
        mocked.run.return_value = sp.CompletedProcess(
        ...
        devices = kbydevices.FastbootDevice.discover()

        assert len(devices) == 1
        assert devices[0].dev_id == serial_no


def test_usb_download(serial_no, fip):
    with mock.patch("flashtool.subprocess") as mocked:
        stdout = b"Sending '/home/tooldev/develop/fip/fip.bin' (687 KB) OKAY [  0.172s]\nFinished. Total time: 0.173s"
        mocked.run.return_value = sp.CompletedProcess(
            args=["./fastboot", "-s", serial_no, "stage", fip],
            returncode=0,
            stdout=stdout
        )
        usb = kbydevices.FastbootDevice(serial_no)
...
def test_pcie_get_devices(serial_no):
    with mock.patch("flashtool.subprocess") as mocked:
        stdout = bytes("Device Found name {}".format(serial_no), "utf-8")
        mocked.run.return_value = sp.CompletedProcess(
            args=["./XLinkDevices"],
            returncode=0,
            stdout=stdout
...
def test_pcie_download(serial_no, fip):
    with mock.patch("flashtool.shutil") as shutil_mocked:
        with mock.patch("flashtool.subprocess") as mocked:
            ...
            pcie = kbydevices.XLinkDevice(serial_no)
