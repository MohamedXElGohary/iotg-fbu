...
g_siip_stitch = os.path.join("..", "siipstitch", "siip_stitch.py")
...
class TestFunctionality(unittest.TestCase):
    ...
    def test_help(self):
        cmd = ["python", g_siip_stitch, "-h"]
        subprocess.check_call(cmd, cwd="tests")

    def test_version(self):
        cmd = ["python", g_siip_stitch, "-v"]
        subprocess.check_call(cmd, cwd="tests")

    def test_replace_pseFw_using_default(self):  # no output file given
        cmd = ["python", g_siip_stitch, "BIOS.bin", "OseFw.bin", "-ip", "pse"]
        subprocess.check_call(cmd, cwd="tests")
        self.assertTrue(
            filecmp.cmp(
                os.path.join("tests", "BIOS_OUT.BIN"),
...
class TestFilesOutsideDir(unittest.TestCase):
    ...
    def test_bios_ip_in_different_dirs(
        self
    ):  # Bios image not in current working directory
        shutil.copy(os.path.join("tests", "BIOS.bin"), os.path.join("tests", "TMPDIR"))
        input1 = os.path.join("TMPDIR", "BIOS.bin")
        os.mkdir(os.path.join("tests", "TMPDIR", "TMPDIR2"))

        shutil.copy(
            os.path.join("tests", "OseFw.bin"),
            os.path.join("tests", "TMPDIR", "TMPDIR2"),
        )
        input2 = os.path.join("TMPDIR", "TMPDIR2", "OseFw.bin")

        cmd = ["python", g_siip_stitch, input1, input2, "-ip", "pse"]
    ...
    def test_bios_in_different_dir(self):  # Bios image not in current working directory
        ...
        cmd = ["python", g_siip_stitch, input1, "OseFw.bin", "-ip", "pse"]
        subprocess.check_call(cmd, cwd="tests")
        self.assertTrue(
            filecmp.cmp(
                os.path.join("tests", "BIOS_OUT.bin"),
    ...
    def test_pseFw_in_different_dir(
        self
    ):  # Ose Firmeware not in current working directory
        shutil.copy(os.path.join("tests", "OseFw.bin"), os.path.join("tests", "TMPDIR"))
        replace = os.path.join("TMPDIR", "OseFw.bin")

        cmd = ["python", g_siip_stitch, "BIOS.bin", replace, "-ip", "pse"]
        subprocess.check_call(cmd, cwd="tests")
        self.assertTrue(
            filecmp.cmp(
                os.path.join("tests", "BIOS_OUT.bin"),
    ...
    def test_key_in_different_dir(self):  # private key not in current working directory
        shutil.copy(
            os.path.join("tests", "priv_key.pem"), os.path.join("tests", "TMPDIR")
        )
...
class TestErrorCases(unittest.TestCase):
    ...
    def test_invalid_ip(self):  # invalid ipname
        cmd = ["python", g_siip_stitch, "BIOS.bin", "OseFw.bin", "-ip", "ose"]
        try:
    ...
    def test_bios_file_do_not_exist(self):  # Bios input file does not exists
        cmd = ["python", g_siip_stitch, "SIIP.bin", "OseFw.bin", "-ip", "pse"]
    ...
    def test_ip_file_does_not_exist(self):  # ip input file does not exists
        cmd = ["python", g_siip_stitch, "BIOS.BIN", "ose.bin", "-ip", "pse"]
    ...
    def test_bios_file_is_empty(self):  # BIOS input file is blank
        cmd = ["python", g_siip_stitch, "BIOS_NUL.BIN", "OseFw.bin", "-ip", "pse"]
    ...
    def test_ip_file_is_empty(self):  # BIOS input file is blank
        cmd = ["python", g_siip_stitch, "BIOS.BIN", "OseFw_NUL.bin", "-ip", "pse"]
    ...
    def test_inputfile_w_incorrect_format(
        ...
        cmd = ["python", g_siip_stitch, "BIOS_badFormat.bin", "OseFw.bin", "-ip", "pse"]
    ...
    def test_no_fv_found(self):  # Firmware volume not found in the file
        cmd = ["python", g_siip_stitch, "BIOS.bin", "dummy_2.bin", "-ip", "oob"]
    ...
    def test_missing_key(self):
        ...
        cmd = ["python", g_siip_stitch, "BIOS.bin", "Vbt.bin", "-ip", "vbt"]
        try:
            subprocess.check_call(cmd, cwd="tests")
            self.fail("a call process eror should have occured")
        except subprocess.CalledProcessError:
...
class TestExceptions(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        shutil.move(
            os.path.join("siipSupport2", "Bin", "Win32"),
